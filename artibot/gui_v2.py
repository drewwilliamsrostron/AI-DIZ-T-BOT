...
class TradingGUI:
    ...
    def update_equity_curve(self, timestamps: list, values: list) -> None:
        """Refresh the equity curve plot with new data."""
        self.ax_equity.clear()
        self.ax_equity.plot(timestamps, values, label="Equity")
        self.ax_equity.legend()
        self.canvas_train.draw()

    def update_best_stats(self, stats: dict) -> None:
        """Update best metric labels from ``stats``."""
        if "sharpe" in stats:
            self.best_sharpe_label.config(text=f"Best Sharpe: {stats['sharpe']:.2f}")
        if "max_dd" in stats:
            self.best_drawdown_label.config(text=f"Best Max DD: {stats['max_dd']:.3f}")
        if "reward" in stats:
            self.best_composite_label.config(text=f"Best Comp: {stats['reward']:.2f}")
        if "epoch" in stats:
            self.best_trades_label.config(text=f"Epoch: {stats['epoch']}")
        self.canvas_train.draw()
