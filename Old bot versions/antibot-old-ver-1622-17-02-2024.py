...
from torch.optim.lr_scheduler import ReduceLROnPlateau  # NEW: LR Scheduler
...
###############################################################################
# Dataset Definition (with StandardScaler normalization and Data Augmentation)
###############################################################################
class HourlyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Data augmentation: add synthetic noise with 20% chance
...
###############################################################################
# Trading Model (Simplified Architecture with increased dropout)
###############################################################################
class TradingModel(nn.Module):
    def __init__(self, input_size=5, hidden_size=64, num_layers=2, num_classes=3, dropout=0.3):
        ...
        # Increased dropout rate in the LSTM
...
###############################################################################
# Ensemble Model (with Mixed Precision, LR Scheduling, and Early Stopping)
###############################################################################
class EnsembleModel:
    def __init__(self, device, n_models=2, lr=3e-4, weight_decay=1e-4):
        self.device = device
        self.models = [TradingModel().to(device) for _ in range(n_models)]
        self.optimizers = [optim.Adam(m.parameters(), lr=1e-4, weight_decay=weight_decay) for m in self.models]
        ...
        # NEW: Learning rate schedulers for each optimizer
    ...
    def train_one_epoch(self, dl_train, dl_val, data_full, stop_event=None):
        # --------------------------------------------------------
        # Run a backtest at the start of the epoch
        # --------------------------------------------------------
        backtest_result = robust_backtest(self, data_full)

        # Store results in globals so the GUI can update
        ...
        # We use composite_reward to guide the reward prediction
        ...
        for batch_x, batch_y in dl_train:
            ...
            for model, opt in zip(self.models, self.optimizers):
                ...
                # Check for invalid gradients (NaN/Inf)
        ...
        # --------------------------------------------------------
        # Evaluate on validation set (if any) + update LR schedulers
        # --------------------------------------------------------
        ...
        # --------------------------------------------------------
        if val_loss is not None:
            # If we improved validation loss
            if val_loss < self.best_val_loss:
                ...
                logging.info(
                    ...
                    f"Net Profit: {backtest_result['effective_net_pct']:.2f}% "
                ...
                # If patience limit exceeded, stop training
                if self.patience_counter >= 15:  # Patience changed from 5 to 15
                    logging.info("Early stopping triggered!")
                    if stop_event is not None:
                        stop_event.set()

        # --------------------------------------------------------
        # Logging for this epoch — fix “val_loss” format
        # --------------------------------------------------------
        ...
        logging.info(
            ...
            f"Net Profit={backtest_result['effective_net_pct']:.2f}% | "
            f"Params: {json.dumps(params_log)}"
        )
    ...
    try:
        ...
        while not stop_event.is_set():
            ...
            if len(data) >= 24:
                ...
                global_attention_weights_history.append(0)  # dummy update

            # Optionally adapt to new live data
            if adapt_to_live:
                data_changed = False
