...
###############################################################################
# Define a NamedTuple for Trade Parameters (must be defined before usage)
###############################################################################
...
squirtle_prompt_template = (
    ...
    "'adjust_take_profit_multiplier' (a float), 'adjust_ATR_period' (an integer), and 'adjust_risk_fraction' (a float). "
    "Metrics:\n{summary}"
)
...
wartortle_prompt_template = (
    ...
    "'adjust_stop_loss_multiplier', 'adjust_take_profit_multiplier', 'adjust_ATR_period', and 'adjust_risk_fraction'.\n"
...
bigmanblastoise_prompt_template = (
    ...
    "- Attention multiplier: 1.0 to 1.5, "
    "- Prediction threshold: 0.05 to 0.15, "
    "- Learning rate: 1e-4 to 1e-2, "
    "- Weight decay: 0.0 to 0.01, "
    "- Stop-loss multiplier: up to 5.0, "
    "- Take-profit multiplier: up to 5.0, "
    "- ATR period: between 5 and 30, "
    "- Risk fraction: between 0.005 and 0.20. "
...
###############################################################################
# Global GPT Memories (re-initialize)
###############################################################################
gpt_memory_squirtle = []
...
# Now outputs both classification logits and learnable trading parameters.
###############################################################################
class TradingModel(nn.Module):
    def __init__(self, input_size=5, hidden_size=64, num_layers=2, num_classes=3, dropout=0.2):
        ...
        # Output dimension: 3 classification logits + 3 trading parameters.
    ...
    def forward(self, x):
        ...
        out_all = self.fc(context)  # shape (bs, 6)
        logits = out_all[:, :3]
        # Transform extra outputs:
        ...
        trade_params = TradeParams(risk_fraction=risk_frac, sl_multiplier=sl_mult, tp_multiplier=tp_mult, attention=w)
        return logits, trade_params

###############################################################################
# Modified Ensemble Model
# The predict function now averages trading parameters from the ensemble.
###############################################################################
class EnsembleModel:
    ...
    def train_one_epoch(self, dl_train, dl_val, data_full):
        ...
        # Use the last 24 bars to get model trading parameters.
        tail24 = data_full[-24:]
    ...
    def predict(self, x):
        ...
        with torch.no_grad():
            ...
            for model in self.models:
                logits, trade_params = model(x.to(self.device))
    ...
    def optimize_models(self, dummy_input):
        ...
        # Pass strict=False to allow tracing with NamedTuple output.
        for model in self.models:
            model.eval()
...
###############################################################################
# CSV Loader
# Uses a regex separator that splits on commas or tabs.
# Skips the first row (the URL) so that the second row becomes the header.
###############################################################################
def load_csv_hourly(csv_path="Gemini_BTCUSD_1h.csv"):
    ...
    try:
        ...
        logging.error(f"Error reading CSV with regex separator: {e}")
        return []
    ...
    df.rename(columns=expected_columns, inplace=True, errors='ignore')
    required_cols = ['unix', 'open', 'high', 'low', 'close']
    for col in required_cols:
        if col not in df.columns:
            logging.error(f"Required column '{col}' not found after parsing. Columns found: {df.columns.tolist()}")
            return []
    ...
    for row in df.itertuples(index=False):
        ...
        v = float(getattr(row, 'volume_btc', 0.0)) / 10000.0
...
###############################################################################
# Main Function and Meta–Control Loop
###############################################################################
