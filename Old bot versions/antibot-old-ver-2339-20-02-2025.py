...
+ Meta–Control via Neural Network (DQN) for Hyperparameter Adjustment
===============================================================================================
DISCLAIMER:
...
###############################################################################
global_SL_multiplier = 5       # fixed SL multiplier (not adjusted by meta–agent)
global_TP_multiplier = 5       # fixed TP multiplier
global_ATR_period = 50         # fixed ATR period
risk_fraction = 0.03
...
# Global best training hyperparameters (to record those that yielded best performance):
global_best_lr = None
global_best_wd = None
...
###############################################################################
# TradingModel (LSTM with SMA, input_size=6)
###############################################################################
...
###############################################################################
# DQN Meta RL Agent (adjusts LR and weight decay)
###############################################################################
class DQNMetaAgent(nn.Module):
    def __init__(self, input_dim=2, output_dim=9, epsilon=0.1, lr=0.001, gamma=0.9):
        ...
        # Action space: each action is a tuple (lr_adj, wd_adj) from {-0.1, 0, 0.1}
        self.action_space = []
        for lr_adj in [-0.1, 0, 0.1]:
            for wd_adj in [-0.1, 0, 0.1]:
                self.action_space.append((lr_adj, wd_adj))
    ...
    def choose_action(self, state_np):
        ...
        q_values = self.forward(state)  # shape (1, 9)
...
class EnsembleModel:
    ...
    def train_one_epoch(self, dl_train, dl_val, data_full, stop_event=None):
        ...
        if current_reward > self.best_composite_reward:
            ...
            logging.info(
                f"NEW BEST reward={current_reward:.2f} | Net Profit={current_result['net_pct']:.2f}% " +
                f"(trades={current_result['trades']}), Sharpe={current_result['sharpe']:.2f}"
            )
        ...
        logging.info(
            f"Epoch {self.train_steps+1} => Train Loss={train_loss:.4f}, Val Loss={val_loss_str}, " +
            f"NetProfit={current_result['net_pct']:.2f}%, Trades={current_result['trades']}, " +
            f"Sharpe={current_result['sharpe']:.2f}, Drawdown={current_result['max_drawdown']:.3f}, " +
            ...
            try:
                ...
                if data_full is not None and len(data_full) > 24:
                    ...
                    logging.info(
                        ...
                        f"CompositeReward={loaded_result['composite_reward']:.2f}, NetPct={loaded_result['net_pct']:.2f}%"
                    )
            except Exception as e:
...
def robust_backtest(ensemble, data_full):
    ...
    # In this implementation, we focus solely on net profit
    yoy_weight = 0.0
    alpha = 1.0
    beta = 0.0
    gamma = 0.0
...
class TradingGUI:
    def __init__(self, root, ensemble):
        ...
        # Since ATR, SL, TP are not adjusted by the meta agent in this version, we show fixed values:
        self.atr_label = ttk.Label(self.info_frame, text=f"ATR Period: {global_ATR_period}", font=("Helvetica", 12))
        self.atr_label.grid(row=6, column=0, sticky=tk.W, padx=5, pady=5)
        self.sl_label = ttk.Label(self.info_frame, text=f"SL Multiplier: {global_SL_multiplier}", font=("Helvetica", 12))
        self.sl_label.grid(row=7, column=0, sticky=tk.W, padx=5, pady=5)
        self.tp_label = ttk.Label(self.info_frame, text=f"TP Multiplier: {global_TP_multiplier}", font=("Helvetica", 12))
        ...
        self.best_lr_label = ttk.Label(self.info_frame, text=f"Learning Rate: N/A", font=("Helvetica", 12), foreground="darkgreen")
        self.best_lr_label.grid(row=5, column=1, sticky=tk.W, padx=5, pady=5)
        self.best_wd_label = ttk.Label(self.info_frame, text=f"Weight Decay: N/A", font=("Helvetica", 12), foreground="darkgreen")
    ...
    def update_dashboard(self):
        ...
        self.sl_label.config(text=f"SL Multiplier: {global_SL_multiplier}")
        self.tp_label.config(text=f"TP Multiplier: {global_TP_multiplier}")
        self.best_lr_label.config(text=f"Learning Rate: {global_best_lr if global_best_lr is not None else 'N/A'}")
        self.best_wd_label.config(text=f"Weight Decay: {global_best_wd if global_best_wd is not None else 'N/A'}")
...
# This meta agent adjusts the learning rate and weight decay.
###############################################################################
def meta_control_loop(ensemble, dataset, agent, interval=1200):
    ...
    global global_composite_reward, global_best_composite_reward, global_best_lr, global_best_wd
    ...
    while True:
        ...
        # Update Weight Decay (clamped between 0 and 1e-2)
        current_wd = ensemble.optimizers[0].param_groups[0].get('weight_decay', 0)
        new_wd = current_wd * (1 + wd_adj)
        ...
        summary = (
            ...
            f"Action taken (lr_adj, wd_adj): {action}\n"
            f"New Hyperparameters -> LR: {new_lr:.2e}, Weight Decay: {new_wd:.2e}\n"
        )
...
def main():
    ...
    meta_agent = DQNMetaAgent(input_dim=2, output_dim=9, epsilon=0.1, lr=0.001, gamma=0.9)
