...
def extract_json(text):
    try:
        ...
        # Fallback to regex for dirty JSON
...
###############################################################################
def query_chatgpt(prompt, memory, model="gpt-4o", temperature=0.3, max_tokens=150, retries=2, backoff_factor=0.5):
    # Include recent memory context (last 3 entries) in the system message
    system_message = (
    ...
    for i in range(retries):
        try:
            ...
            # Verify that the response contains a JSON object
...
class TradingModel(nn.Module):
    def __init__(self, input_size=5, hidden_size=64, num_layers=2, num_classes=3, dropout=0.2):
        ...
        # Increase output dimension by 1 for composite reward prediction.
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True,
                            dropout=dropout if num_layers > 1 else 0)
        self.layernorm = nn.LayerNorm(hidden_size)
        self.attn = nn.Linear(hidden_size, 1)
        self.dropout = nn.Dropout(dropout)
        # Final layer outputs 3 classification logits, 3 trade parameters, and 1 composite reward.
        self.fc = nn.Linear(hidden_size, num_classes + 3 + 1)
    def forward(self, x):
        bs = x.size(0)
        dev = x.device
        ...
        pred_reward = out_all[:, 6]  # predicted composite reward
...
class EnsembleModel:
    def __init__(self, device, n_models=2, lr=3e-5, weight_decay=0.0):
        ...
        # Weight for the auxiliary reward prediction loss.
        self.reward_loss_weight = 0.1  
    def train_one_epoch(self, dl_train, dl_val, data_full):
        # Compute target composite reward using a full backtest.
        backtest_result = robust_backtest(self, data_full)
        composite_reward_target = torch.tensor(backtest_result["composite_reward"], dtype=torch.float32, device=self.device)
        ...
        for batch_x, batch_y in dl_train:
            ...
            for model, opt in zip(self.models, self.optimizers):
                ...
                reward_target = composite_reward_target.expand_as(pred_reward)
    ...
    def evaluate_val_loss(self, dl_val):
        for m in self.models:
            m.train()  # we compute loss in training mode (with dropout) if desired
        losses = []
        with torch.no_grad():
    ...
    # Prediction functions use no_grad but do not switch modes.
    def predict(self, x):
        with torch.no_grad():
            outs = []
            for m in self.models:
                logits, _, pred_reward = m(x.to(self.device))
    ...
    def vectorized_predict(self, windows_tensor):
        with torch.no_grad():
            ...
            trade_params_list = []
            for m in self.models:
                logits, trade_params, _ = m(windows_tensor.to(self.device))
                probs = torch.softmax(logits, dim=1)
                outs.append(probs)
                if isinstance(trade_params, tuple):
                    risk_frac_val = trade_params[0]
                    sl_mult_val = trade_params[1]
                    tp_mult_val = trade_params[2]
                else:
                    risk_frac_val = trade_params.risk_fraction
                    sl_mult_val = trade_params.sl_multiplier
                    tp_mult_val = trade_params.tp_multiplier
                trade_params_list.append({
                    "risk_fraction": risk_frac_val.cpu(),
                    "sl_multiplier": sl_mult_val.cpu(),
                    "tp_multiplier": tp_mult_val.cpu(),
                })
            avg_probs = sum(outs) / len(outs)
            pred_indices = avg_probs.argmax(dim=1)
            confs = avg_probs.max(dim=1)[0]
            avg_trade_params = {}
            for key in ["risk_fraction", "sl_multiplier", "tp_multiplier"]:
                param_tensor = torch.stack([tp[key] for tp in trade_params_list], dim=0)
                avg_trade_params[key] = param_tensor.mean(dim=0).cpu()
        return pred_indices.cpu(), confs.cpu(), avg_trade_params

    # IMPORTANT: Use torch.jit.script instead of torch.jit.trace for robust optimization.
    def optimize_models(self, dummy_input):
        self.optimized_models = []
        for m in self.models:
            # Script the model instead of tracing to avoid dynamic control-flow issues.
            m_script = torch.jit.script(m)
            self.optimized_models.append(m_script)
        logging.info("Models optimized with TorchScript (scripted).")
...
###############################################################################
# Robust Backtest Function (with Inactivity Penalty, Composite Reward, & Days in Profit)
###############################################################################
def robust_backtest(ensemble, data_full):
    ...
    pred_indices, _, _ = ensemble.vectorized_predict(windows_tensor)
    ...
    if trades:
        ...
        if total_duration > inactivity_threshold:
            ...
            inactivity_penalty = days_without_trading * inactivity_penalty_weight
    sharpe_ratio = 0.0
    composite_reward = net_pct - inactivity_penalty
...
def main():
    ...
    # Ensure cudnn is in deterministic mode.
