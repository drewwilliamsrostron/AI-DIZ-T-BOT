...
# We'll store all the backtest metrics in globals to show in the GUI:
global_sharpe = 0.0
...
# NEW: We'll store an ASCII table of year–by–year performance:
...
moneymaker_prompt_template = (
    ...
    "DO NOT INCLUDE MARKDOWN, CODE BLOCKS, OR EXTRA TEXT.\n"
    "Metrics:\n{summary}\nSquirtle: {squirtle_response}\nWartortle: {wartortle_res}\nBigManBlastoise: {bigmanblastoise_res}"
)
...
def moneymaker_controller(ensemble, summary, squirtle_res, wartortle_res, bigmanblastoise_res):
    prompt = moneymaker_prompt_template.format(
        ...
        wartortle_res=wartortle_res["response"],
        bigmanblastoise_res=bigmanblastoise_res["response"],
        summary=summary
    )
...
def apply_adjustments(adjustments):
    ...
    if not isinstance(adjustments, dict):
        ...
        return
    # Enforce safety limits for adjustments using np.clip
    if "adjust_stop_loss_multiplier" in adjustments:
        global_SL_multiplier = np.clip(float(adjustments["adjust_stop_loss_multiplier"]), 0.2, 3.0)
        logging.info(f"Stop Loss multiplier adjusted to: {global_SL_multiplier}")
    if "adjust_take_profit_multiplier" in adjustments:
        global_TP_multiplier = np.clip(float(adjustments["adjust_take_profit_multiplier"]), 1.0, 5.0)
    ...
    if "adjust_risk_fraction" in adjustments:
        risk_fraction = np.clip(float(adjustments["adjust_risk_fraction"]), 0.01, 0.2)
...
###############################################################################
# Dataset Definition (with StandardScaler + Data Augmentation)
###############################################################################
...
###############################################################################
# YEAR-BY-YEAR STATS HELPER
###############################################################################
def compute_yearly_stats(equity_curve, trades, initial_balance=10000.0):
    """
    Returns a DataFrame and an ASCII table string with per-year:
      - net profit (%)
      - Sharpe
      - max drawdown
      - number of trades
    ...
    # We'll also build a trades_df by year
    ...
    for year, grp in grouped:
        ...
        if sigma > 1e-12:
            sharpe_year = (mu * 252) / (sigma * np.sqrt(252))
        else:
            sharpe_year = 0.0
    ...
    df_yearly = pd.DataFrame(year_rows)
    df_yearly.set_index("Year", inplace=True)
    # Format an ASCII table
    table_str = df_yearly.to_string(
...
class EnsembleModel:
    ...
    def train_one_epoch(self, dl_train, dl_val, data_full, stop_event=None):
        # Run backtest at start of epoch
        backtest_result = robust_backtest(self, data_full)

        # NEW: Save backtest metrics to global variables for GUI
        ...
        global_equity_curve = backtest_result["equity_curve"]
        global_backtest_profit.append(backtest_result["effective_net_pct"])
        global_inactivity_penalty = backtest_result["inactivity_penalty"]
        global_composite_reward = backtest_result["composite_reward"]
        global_days_without_trading = backtest_result["days_without_trading"]
        global_trade_details = backtest_result["trade_details"]
        global_days_in_profit = backtest_result["days_in_profit"]

        global_sharpe = backtest_result["sharpe"]
        global_max_drawdown = backtest_result["max_drawdown"]
        global_net_pct = backtest_result["net_pct"]
        global_num_trades = backtest_result["trades"]

        # (A) Compute yearly stats and produce ASCII table
        df_yearly, table_str = compute_yearly_stats(
            backtest_result["equity_curve"],
            backtest_result["trade_details"],
            initial_balance=10000.0
        )
        # (B) Save table for console & GUI
        ...
        # Scale our reward target
        scaled_target = torch.tanh(
            torch.tensor(
                backtest_result["composite_reward"] / 100,
                dtype=torch.float32,
                device=self.device
            )
        )
        ...
        if val_loss is not None:
            if val_loss < self.best_val_loss:
                ...
                logging.info(
                    ...
                    f"Net Profit: {backtest_result['net_pct']:.2f}% "
                    f"(raw: {backtest_result['net_pct']:.2f}%, trades: {backtest_result['trades']}). "
                    f"Inactivity Penalty: {backtest_result['inactivity_penalty']:.2f}, "
                    f"Days without Trading: {backtest_result['days_without_trading']:.2f}, "
                    f"Composite Reward: {backtest_result['composite_reward']:.2f} "
                    f"Sharpe={backtest_result['sharpe']:.2f}, MDD={backtest_result['max_drawdown']:.3f}"
                )
                ...
                logging.info(f"Validation loss did not improve. Patience counter: {self.patience_counter}")
        ...
        # Additional logging to show all metrics each epoch
        logging.info(
            f"Epoch {self.train_steps+1} => "
            f"Train Loss={train_loss:.4f}, "
            f"Val Loss={val_loss_str}, "
            f"NetProfit={backtest_result['net_pct']:.2f}%, "
            f"Trades={backtest_result['trades']}, "
            f"Sharpe={backtest_result['sharpe']:.2f}, "
            f"Drawdown={backtest_result['max_drawdown']:.3f}, "
            f"Inactivity={backtest_result['inactivity_penalty']:.2f}, "
            f"CompositeReward={backtest_result['composite_reward']:.2f}"
        )
...
###############################################################################
# Robust Backtest (with multi-metric reward)
###############################################################################
def robust_backtest(ensemble, data_full):
    """
    A multi-objective backtest that:
      - calculates net profit (net_pct)
      - Sharpe ratio (daily)
      - max drawdown
      - inactivity penalty
      - # of trades reward
      - => returns a 'composite_reward'
    Also sets 'effective_net_pct' = net_pct for the user's graph.
    """
    if len(data_full) < 24:
    ...
    inactivity_penalty_weight = 0.01
    ...
    sharpe_score = sharpe
    drawdown_penalty = abs(max_drawdown_val)
...
class TradingGUI:
    def __init__(self, root, ensemble):
        ...
        self.drawdown_label = ttk.Label(self.info_frame, text="Max DD: N/A", font=("Helvetica", 12))
        self.drawdown_label.grid(row=5, column=0, sticky=tk.W, padx=5, pady=5)

        self.netprofit_label = ttk.Label(self.info_frame, text="Net Profit (%): N/A", font=("Helvetica", 12))
        self.netprofit_label.grid(row=6, column=0, sticky=tk.W, padx=5, pady=5)

        self.trades_label = ttk.Label(self.info_frame, text="Trades: N/A", font=("Helvetica", 12))
        self.trades_label.grid(row=7, column=0, sticky=tk.W, padx=5, pady=5)

        self.inactivity_label = ttk.Label(self.info_frame, text="Inactivity Penalty: N/A", font=("Helvetica", 12))
        self.inactivity_label.grid(row=8, column=0, sticky=tk.W, padx=5, pady=5)

        self.composite_label = ttk.Label(self.info_frame, text="Composite Reward: N/A", font=("Helvetica", 12))
        self.composite_label.grid(row=9, column=0, sticky=tk.W, padx=5, pady=5)

        self.days_profit_label = ttk.Label(self.info_frame, text="Days in Profit: N/A", font=("Helvetica", 12))
        self.days_profit_label.grid(row=10, column=0, sticky=tk.W, padx=5, pady=5)
    ...
    def update_dashboard(self):
        ...
        # Display year-by-year table in the new tab
        ...
        self.sharpe_label.config(text=f"Sharpe Ratio: {global_sharpe:.2f}")
        self.drawdown_label.config(text=f"Max DD: {global_max_drawdown:.3f}")
        self.netprofit_label.config(text=f"Net Profit (%): {global_net_pct:.2f}")
        self.trades_label.config(text=f"Trades: {global_num_trades}")

        if global_inactivity_penalty is not None:
            self.inactivity_label.config(text=f"Inactivity Penalty: {global_inactivity_penalty:.2f}")
        else:
            self.inactivity_label.config(text="Inactivity Penalty: N/A")

        if global_composite_reward is not None:
            self.composite_label.config(text=f"Composite Reward: {global_composite_reward:.2f}")
        else:
            self.composite_label.config(text="Composite Reward: N/A")

        if global_days_in_profit is not None:
            self.days_profit_label.config(text=f"Days in Profit: {global_days_in_profit:.2f}")
        else:
            self.days_profit_label.config(text="Days in Profit: N/A")
...
# We'll store the ASCII table of year-by-year stats
global_yearly_stats_table = ""

# Additional live queue
